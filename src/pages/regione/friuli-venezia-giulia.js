...
function FriuliVeneziaGiulia(values) {
      ...
      {isLoaded ? (
        ...
        <>
          ...
          <div className="RegionDetails">
            <span className="RegionName">Friuli Venezia Giulia</span>
            <Share />
...
export default () => (
  <StaticQuery
    query={graphql`
      query FriuliVeneziaGiulia {
        allRtCsv(filter: { state: { eq: "Friuli Venezia Giulia" } }) {
          totalCount
          edges {
            node {
              id
    ...
    `}
    render={data => <FriuliVeneziaGiulia data={data} />}
  />
)
